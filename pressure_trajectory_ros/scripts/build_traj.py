...
class trajBuilder:
    def __init__(self, filename):
        ...
        # Get the trajectory settings from the parameter server
        all_settings = rospy.get_param(rospy.get_name())
        self.settings = all_settings.get("settings",None)
    ...
    def saveOut(self, outTraj):
        ...
        outFile=os.path.join(curr_path,"..","traj_generated",self.filename+"_raw.yaml")
...
if __name__ == '__main__':
    try:
        rospy.init_node('pressure_trajectory_builder')
        profile = rospy.get_param(rospy.get_name()+'/traj_profile')
        build = trajBuilder(profile)
        build.go()
